...
def _selector_parts(selector: str) -> tuple[list[str], list[str]] | None:
    """Parse the small, deterministic selector subset needed by the validator.

    Return compounds and the combinator between each pair, or None when the
    selector needs a full browser CSS engine. Sibling combinators and pseudo
    selectors are intentionally not guessed.
    """
    ...
    for token in tokens:
        if token == ">":
            if expect_compound or combinators and len(combinators) >= len(compounds):
                return None
            combinators.append(">")
            expect_compound = True
            pending_space = False
            continue
        if not expect_compound and len(compounds) > len(combinators):
            combinators.append(" ")
...
def _compound_matches(compound: str, attrs: dict[str, str], tag: str | None) -> bool:
    if not compound:
        return False
    remainder = compound
    tag_match = re.match(r"^(?:[A-Za-z][\w-]*|\*)", remainder)
    if tag_match:
        wanted_tag = tag_match.group(0)
        remainder = remainder[tag_match.end():]
        if wanted_tag != "*" and (tag or "").lower() != wanted_tag.lower():
            return False
    elif not re.match(r"^(?:[.#\[])", remainder):
        return False
    wanted_id = None
    wanted_classes: list[str] = []
    requires_root = False
    consumed = 0
    for match in re.finditer(r"#[\w-]+|\.[\w-]+|\[[^\]]+\]", remainder):
        ...
        if token.startswith("#"):
            ...
            wanted_classes.append(token[1:])
        elif token == "[data-ui-root]":
            requires_root = True
        else:
            return False
        consumed = match.end()
    if consumed != len(remainder):
        return False
    if wanted_id is not None and attrs.get("id") != wanted_id:
        return False
    classes = set(attrs.get("class", "").split())
    if any(class_name not in classes for class_name in wanted_classes):
        return False
    return not requires_root or "data-ui-root" in attrs


def selector_matches_node(
    selector: str,
    attrs: dict[str, str],
    tag: str | None = None,
    ancestry: list[tuple[str, dict[str, str]]] | None = None,
) -> bool:
    """Match supported selectors right-to-left without treating descendants as siblings."""
    ancestry = ancestry or []
    for alternative in selector.split(","):
        parsed = _selector_parts(alternative)
        if parsed is None:
            continue
        compounds, combinators = parsed
        if not _compound_matches(compounds[-1], attrs, tag):
            continue
        current_index = 0
        matched = True
        for index in range(len(compounds) - 2, -1, -1):
            ...
            if combinator == ">":
                if current_index >= len(ancestry):
                    matched = False
                    break
                parent_tag, parent_attrs = ancestry[current_index]
                current_index += 1
                if not _compound_matches(compounds[index], parent_attrs, parent_tag):
                    matched = False
                    break
            else:
                found = False
                while current_index < len(ancestry):
                    parent_tag, parent_attrs = ancestry[current_index]
                    current_index += 1
                    if _compound_matches(compounds[index], parent_attrs, parent_tag):
                        found = True
                        break
                if not found:
                    matched = False
...
def declarations_for_element(
    ...
    for selector, rule in css_rules:
        if selector_matches_node(selector, attrs, tag, ancestry):
...
class DeliveryHTMLParser(HTMLParser):
    def __init__(self) -> None:
        ...
        self.node_ancestry: dict[str, list[tuple[str, dict[str, str]]]] = {}
    ...
    def handle_starttag(self, tag: str, attrs: list[tuple[str, str | None]]) -> None:
        ...
        if "data-node-id" in attrs_dict:
            ...
            self.node_ancestry[explicit] = list(reversed(self.element_stack[:-1]))
        ...
        if tag == "svg":
            ...
            self.svg_occurrences.append({
                ...
                "parentAncestry": list(reversed(self.element_stack[:-1])) if parent_element else [],
            })
...
def _svg_geometry_from_file_cached(path_string: str, mtime_ns: int) -> tuple[tuple[str, tuple[tuple[str, str], ...]], ...]:
    ...
    try:
        ...
        return ()
    return tuple(parser.svg_geometry.get("icon", []))
...
def svg_geometry_from_file(path: Path) -> list[tuple[str, tuple[tuple[str, str], ...]]]:
    try:
        ...
        return []
    return list(_svg_geometry_from_file_cached(str(path), mtime_ns))


def error_category(message: str) -> str:
    """Classify diagnostics without changing validation severity or behavior."""
    lowered = message.lower()
    if any(token in lowered for token in ("verification.visual", "visual evidence", "visual inspection")):
        return "visual"
    if any(token in lowered for token in ("asset", "lucide", "image fill", "raster", "vector")):
        return "asset"
    if any(token in lowered for token in ("html", "svg", "viewport", "css", "stylesheet", "data-url", "root")):
...
def validate(
    ...
    if html_path is not None:
        ...
        for selector, declarations in css_rule_blocks(styles):
            matched_nodes = [
                node_id for node_id, attrs in parser.node_attributes.items()
                if selector_matches_node(selector, attrs, parser.node_tags.get(node_id), parser.node_ancestry.get(node_id))
            ]
        ...
        if layout_mode not in {"uniform-scale", "reflow", "fixed"}:
            ...
            for selector, declarations in css_rule_blocks(styles):
                if selector_matches_node(
                    selector,
                    root_attrs,
                    parser.node_tags.get(root_id, ""),
                    parser.node_ancestry.get(root_id),
                ):
            ...
            for occurrence in parser.svg_occurrences:
                ...
                if not isinstance(owner_node_id, str) or owner_node_id not in node_by_id:
                    ...
                    continue
                svg_declarations = declarations_for_element(
                    css_rules,
                    occurrence["attrs"],
                    "svg",
                    occurrence.get("parentAncestry", []),
                )
                ...
                if not isinstance(parent_tag, str) or parent_tag == "svg":
                    ...
                    continue
                parent_declarations = declarations_for_element(
                    css_rules,
                    parent_attrs,
                    parent_tag,
                    occurrence.get("parentAncestry", [])[1:],
                )
