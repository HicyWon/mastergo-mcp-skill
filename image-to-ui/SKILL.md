...
4. Lock the single final UI image as the reference artifact and ask for the user's opinion. Before presenting this confirmation, read [references/confirmation-controls.md](references/confirmation-controls.md) and always offer its visual-reference actions as complete plain-text choices. Treat plain text as the only default confirmation protocol; use a `visualize` fragment only when the user explicitly requests interactive controls or the active runtime explicitly declares directive support, and never allow a failed fragment to duplicate the text choices. Do not decompose until the user confirms the visual reference. Preserve the complete, uncropped source image at its original pixel dimensions and record those dimensions before any asset extraction. Keep four objects distinct throughout the task: the immutable locked reference image; the cleaned full-canvas `参考保真视觉底图`; bounded independent image assets; and editable HTML/canvas nodes. Never use a cropped derivative as the visual-base source. Any regenerated, restyled, or materially edited UI image becomes a new locked reference and invalidates the previous decomposition.
...
7. After presenting the decomposition and synchronized draft, read [references/confirmation-controls.md](references/confirmation-controls.md) and always offer its four decomposition-confirmation/output actions as complete plain-text choices. Keep those choices as the only default user-visible confirmation protocol; enable an optional `visualize` panel only under the conditions in that reference. A previously named target is a suggested default only; an explicit current choice overrides it. Do not make HTML a default approval gate.
...
After writing an HTML preview, save it as an immutable provisional revision before validation, record its path, revision, and SHA-256 hash, and immediately report the path as `provisional` with the meaning “HTML 预览已生成，正在校验。” Never overwrite a provisional revision during repair. Run one fast contract validation and one browser screenshot/visual-anchor check for the revision. Report a final delivered result only after both pass; if validation fails, retain the original path and label it `provisional / 校验未通过`, classify the failures, and stop by default. Allow at most one new revision for a single unambiguous, local repair; revalidate and rescreenshot that new revision, then keep it only if it is not visually worse than the prior screenshot. Never use validator success as a substitute for visual completion or change confirmed scaling anchors merely to satisfy a contract error.
...
Resolve the script path from the directory containing this `SKILL.md`. For HTML, do not run an unbounded repair loop: validate the canonical IR/HTML contract once for the provisional revision, inspect one browser screenshot including a reduced-width uniform-scale state, and only rerun validation after the single allowed repair. If a final visual record is required, include it in the same final gate invocation or perform only a lightweight record check; do not repeat the full contract scan solely because a screenshot was recorded. A non-zero exit blocks completion. The script validates tree integrity, child-level presentation lineage, route locking, real asset files and metadata, exact data-URL bytes, mapped text/nodes, self-containment, root-canvas ratio, strict source/target normalization, Lucide metadata, target-mapping completeness, and the presence of visual evidence. It does not judge visual similarity or semantic image cleanliness; the agent must inspect the evidence against the locked reference and reject any repair that changes scaling, typography, icon geometry, navigation placement, or composite-control alignment.
