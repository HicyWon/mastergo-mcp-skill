...
- resolve the validator relative to the active image-to-ui skill directory, then run `python3 "/absolute/path/to/image-to-ui/scripts/validate-delivery.py" --ir /absolute/output/ui-ir.json --html /absolute/output/index.html` for HTML, or `--target <adapter>` before a canvas write; a non-zero exit blocks delivery;
...
Use deterministic validation, one primary write, one structural readback, and one inspected browser/target screenshot. Record `verification.visual.status: "inspected"` and an evidence reference. Do not mark completion without this evidence.
...
Make at most one focused repair for an obvious omission, unsupported icon, missing image fill, or clearly broken major region. Ask the user before changing meaning, text, reference-anchor assets, or product scope. If the repair is not unambiguous or does not work, report the difference and stop.
