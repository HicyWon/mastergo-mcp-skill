...
- The plain-text choices are the only default user-visible confirmation protocol. Do not create a visualization fragment or emit `::codex-inline-vis` by default. This keeps confirmation reliable across clients that render unsupported directives literally.
- Enable a compact `visualize` confirmation fragment only when the user explicitly requests interactive controls or the active runtime explicitly declares support for the directive. If enabled, create at most one current-stage fragment and ensure a failed or closed fragment does not render a second competing copy of the confirmation UI. The text choices remain the canonical protocol and must not be duplicated by a fallback fragment.
...
- When `visualize` is unavailable, compatibility is uncertain, or the user did not request controls, omit the fragment and directive and continue with text alone. Do not emit an unsupported control directive.
